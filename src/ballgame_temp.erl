...
handle_call(run, _From, State) ->
    ...
    Id = ballgame_util:declare_crdt(temp, state_awset),
    ballgame_util:insert_timed_key(State#state.table, pmod_nav:read(acc, [out_temp])),
...
handle_call(store_temp, _From, State) ->
    ballgame_util:insert_timed_key(State#state.table, pmod_nav:read(acc, [out_temp])),
    % ballgame_util:insert_timed_key(State#state.table, get_temp()).
    {reply, ok, State};

%%--------------------------------------------------------------------

handle_call({table, Name}, _From, State) ->
    T = ballgame_util:table(Name),
...
handle_call({declare, Name, Type}, _From, State) ->
    logger:log(notice, "DECLARED ~n"),
    Id = ballgame_util:declare_crdt(Name, Type),
    {reply, ok, State#state{crdt = Id}};
